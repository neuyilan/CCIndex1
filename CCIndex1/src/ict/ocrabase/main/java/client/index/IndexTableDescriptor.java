...
public class IndexTableDescriptor {
	...
	public IndexTableDescriptor(HTableDescriptor tableDescriptor) throws IOException {
		...
		if (Bytes.compareTo(descriptor.getName(),TableName.META_TABLE_NAME.getName()) == 0
				|| Bytes.compareTo(descriptor.getName(), TableName.NAMESPACE_TABLE_NAME.getName()) == 0) {
			throw new IllegalArgumentException("Table name can't be "
					+ TableName.META_TABLE_NAME + " or "
